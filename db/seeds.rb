...
# list countries to import data Country: name, alpha2
# [
...
# ].each do |c|
#   Country.find_or_create_by_name(:name => c[:name], :alpha2 => c[:iso_two_letter_code])
# end

# City(id: integer, name: string, lat: string, lng: string, country_id: integer, geoname_id: integer, created_at: datetime, updated_at: datetime)

COUNTRIES_FILE = File.dirname(__FILE__) + '/data/cities15000.txt'
ROW_HEADINGS = [
...
csv = open(COUNTRIES_FILE)

# csv.each_line do |line|
#   row = line.split("\t")
# 
#   City.seed(:geoname_id) do |c|
#     c.geoname_id = row[ROW_HEADINGS.index(:geonameid)]
# 
#     c.name = if row[ROW_HEADINGS.index(:asciiname)].blank?
#       row[ROW_HEADINGS.index(:name)]
#     else
#       row[ROW_HEADINGS.index(:asciiname)]
#     end
# 
#     c.lat  = row[ROW_HEADINGS.index(:latitude)]
#     c.lng  = row[ROW_HEADINGS.index(:longitude)]
#     c.country = Country.find_by_alpha2(row[ROW_HEADINGS.index(:country_code)])
#   end
# end


# Country(id: integer, name: string, alpha2: string, created_at: datetime, updated_at: datetime, lat: string, lng: string)

csv.each_line do |line|
  row = line.split("\t")

  if row[ROW_HEADINGS.index(:asciiname)].blank?
    city = City.find_by_name(row[ROW_HEADINGS.index(:name)])
  else
    city = City.find_by_name(row[ROW_HEADINGS.index(:asciiname)])
  end    
